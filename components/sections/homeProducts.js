  ...
  useEffect(() => {
    if (!isPaused) {
      const animate = () => {
        setCurrentX(prev => {
          const newX = prev - 1
          // Reset position when first set of products is completely scrolled
          if (newX <= -100 * products.length) {
            return 0
          }
          return newX
        })
      ...
      <div className="mx-auto px-6 lg:px-8">
        ...
        {isPaused && (
          <motion.div
            ...
            className="fixed bottom-8 right-8 bg-[#ff4f01] text-white px-4 py-2 rounded-full shadow-lg z-50"
          >
            <div className="flex items-center space-x-2">
              <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                <path d="M6 4h4v16H6V4zm8 0h4v16h-4V4z"/>
              </svg>
              <span className="text-sm font-medium">Slider Paused</span>
            </div>
          </motion.div>
        )}
      </div>
